...
import { createCustomPaymasterWithNoData, Paymaster, PaymasterKind } from "./paymasters";
...
export const toSharedAccountClient = async (publicClient: PublicClient, paymasterInfo: Paymaster, nonceKey: bigint) => {    
    ...
    const pimlicoClient = createPimlicoClient({
      ...
      transport: http(paymasterInfo.bundlerUrl),
      entryPoint: {
        address: entryPoint07Address,
        version: "0.7",
      },
    ...
    const paymaster =
    paymasterInfo.kind == PaymasterKind.PIMLICO_ERC20
        ? pimlicoClient
        : createCustomPaymasterWithNoData(pimlicoClient, paymasterInfo);

    // Client used to fill userOp object with data, and send requests to the bundler
    return createSmartAccountClient({
      account: sharedAccount,
      chain: publicClient.chain,
      bundlerTransport: http(paymasterInfo.bundlerUrl),
      paymaster,
      userOperation: {
