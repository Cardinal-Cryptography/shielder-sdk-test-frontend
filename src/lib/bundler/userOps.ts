...
export const sendUnshieldTransaction = async (
    smartAccountClient: SmartAccountClient,
    shielderAddress: `0x${string}`,
    args: [`0x${string}`, bigint, bigint, `0x${string}`, bigint, bigint, bigint, `0x${string}`, `0x${string}`, bigint]
) => {
    // Send the userOp
    console.log("Sending user op");
    const tx_hash = await smartAccountClient.sendTransaction({
        calls: [
            {
                to: shielderAddress,
                abi,
