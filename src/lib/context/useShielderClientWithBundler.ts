...
import { PaymasterKind, paymasters } from "../bundler/paymasters";
...
export const useShielderClientWithBundler = () => {
  ...
  const { data: shielderClient, error } = useQuery({
    ...
    queryFn: () => {
      ...
      const pk = deriveShielderPrivateKey(
        seedMnemonicConfig.shielderSeedMnemonic,
      );

      // use the free gas paymaster
      const paymaster = paymasters[chainId].find((p) => p.kind == PaymasterKind.FREE_GAS)!;
